...
if __name__ == "__main__":
    ...
    with open(
        path + "\\pokemon_species_flavor_text_updated.csv",
        "w",
        encoding="utf-8",
        newline="",
    ) as fCSV:
        ...
        for i in range(0, len(goodFiles), 2):
            ...
            if file1 == file2:
                try:
                    ...
                    textFile = TextFile(
                        path + "\\" + goodFiles[i], path + "\\" + goodFiles[i + 1]
                    )
                ...
                try:
                    ...
                    fileName = (
                        os.path.basename(path + "\\" + goodFiles[i]).lower().split("_")
                    )
                    ...
                    for label, flavor_text in dictionary.items():
                        if len(flavor_text) > 1 and "[VAR]" not in flavor_text:
                            ...
                            entriesList.append(
                                [species, version, language, flavor_text]
                            )
