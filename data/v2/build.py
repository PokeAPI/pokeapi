...
def _build_pokemons():
    ...
    def csv_record_to_objects(info):
        yield PokemonFormMethod(id=int(info[0]), name=info[1])

    build_generic((PokemonFormMethod,), "pokemon_form_methods.csv", csv_record_to_objects)

    def csv_record_to_objects(info):
        yield PokemonFormCondition(
            pokemon_form_id=int(info[0]),
            form_method_id=int(info[1]),
